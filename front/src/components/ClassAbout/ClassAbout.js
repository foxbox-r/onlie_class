...
const ClassAbout = ({
    ...
    classStore,titleRef,isCanCreate
}) => {
    return (
            ...
            <div className="classAbout-wrapper">
                ...
                <article className="classAbout-wrapper-article">
                    ...
                    <article className="classAbout-wrapper-article-article">
                        {isCanCreate && <div className="create">
                            <div className="create-profile">{aboutClass.owner.name[0].toUpperCase()}</div>
                            {createSubjectMode
                            ?<div className="create-input">
                                <div className="create-input-text"><CustomInput ref={titleRef} enterEvent={titleEnterEvenet} value={classStore.isCreateSubjectLoading?"Creating..":title} setValue={setTitle} placeholder="제목을 입력하세요." /></div>
                        ...
                        <div className="subjects">{subjects.length?classStore.currentSubjects.map(v=><SubjectItem key={v.id} subject={v} />):<h1>수업을 만드세요</h1>}</div>
                    </article>
                </article>
            </div>
        </div>
